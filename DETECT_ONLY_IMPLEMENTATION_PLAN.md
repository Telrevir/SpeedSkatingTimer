...
> 当前分支首轮实现已完成，剩余状态以 `CurrentTask.md` 的“待实机验证”为准。下方勾选仅表示代码和文档层面的实现项已落地，不代表已经完成 STM32/APP/RFID 实机联调。

**Goal:** 将现有完整计时固件精简为只负责控制RFID、8秒EPC去重和LoRa上报的DetectOnly固件。

**Architecture:** 保留RFIDReader的硬件读写能力；新增无动态内存的DetectionController负责业务状态；重写LoraManager和主程序，暴露确认后的DetectOnly字节协议命令。

**Tech Stack:** STM32 Arduino Core、STM32 HardwareSerial、现有RFID串口协议、LoRa串口透传。

## Global Constraints

- 协议只允许`0x01`、`0x02`、`0x03`、`0x04`、`0x10`、`0x11`、`0x12`、`0x13`、`0x14`、`0xF0`。
- 使用四张各50槽的固定表维护临时运动员、运动员成绩、黑名单和普通EPC 8秒记录。
- 运动员和普通EPC分别执行8秒静默规则。
- 时间使用uint24大端序百分秒并在`0xFFFFFF`饱和。
- 支持临时运动员列表、运动员成绩列表、圈数、单圈时长和总时长；不使用本地存储、姓名、排名、领滑或重置命令。
...
- [x] 编写覆盖首次EPC、8秒静默、满8秒、50槽容量、重复开始和时间饱和的失败测试。
- [x] 运行测试或结构检查，确认缺少实现时失败。
- [x] 使用固定数组完成最小实现，不使用`String`和堆分配。
- [x] 运行测试并确认通过。
...
- [x] 编写当前DetectOnly命令ID、载荷长度、校验和、非法命令和错误状态的失败测试。
- [x] 重写接收状态机和发送接口，删除全部运动员及文本协议代码。
- [x] 验证协议示例字节与文档一致。
...
- [x] 为队列清空、队列溢出状态和主业务引用边界编写失败结构测试。
- [x] 增加显式队列清空/溢出查询接口。
- [x] 重写主循环，仅在检测中读取RFID并处理EPC。
- [x] 确认重复开始不改变运行状态，8秒重复不调用任何输出。
...
- [x] 删除旧模块和不再适用的主业务引用。
- [x] 静态扫描确认不存在旧协议ID、存储库、运动员或领滑符号。
- [x] 运行全部自测试和`git diff --check`。
- [x] 若本机存在可用STM32 Arduino构建工具则编译；否则明确记录未编译和未实机验证。
