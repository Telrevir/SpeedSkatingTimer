...
将小程序比赛历史从“本地完整保存并在启动时全量双向同步”调整为“后端作为在线比赛和历史比赛的权威来源”。小程序只长期保存运动员、分组、未完成的活动比赛、离线比赛以及尚未确认的后台发送任务。
...
- 运动员和分组仍保存在本地，但任何用户操作必须获得后端成功回执后才提交到本地；
- 后台写入、前台查询和蓝牙计时互不阻塞；
- 重试不会重复创建比赛或成绩。
...
运动员目录和分组继续完整保存在现有本地仓库。比赛相关本地存储调整为：
...
1. 分页读取当前俱乐部的运动员、分组和分组关系；成功后更新本地缓存，失败时保留现有本地数据并显示同步提示；
2. 恢复持久 outbox；
...
运动员和分组页面使用“远端确认后本地提交”的事务流程：

1. 根据用户输入构造候选数据，不立即改变现有本地仓库；
2. 通过 `control` 通道调用后端；
3. 等待后端成功回执并严格校验对象身份和内容；
4. 成功后一次性提交本地修改并刷新页面；
5. 超时或错误时丢弃候选数据，保持本地原状并显示明确提示。

运动员使用现有单资源 CRUD。分组名称和成员关系必须作为一个事务保存，后端新增分组聚合写接口，避免多个成员请求中途失败导致远端半完成。归档和恢复也遵循相同规则。

如果后端已成功但小程序本地写入异常，小程序显示“服务器已保存，本地刷新失败”，并立即重新拉取该资源恢复一致，不伪装成后端失败。
...
运动员姓名和 EPC 从本地运动员目录按 `AthleteID` 关联。目录中不存在的运动员使用 ID 占位，不丢弃成绩。请求失败时保留最近一次成功画面并显示非阻塞错误提示。
